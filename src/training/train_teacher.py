"""Script di training per il Teacher Visivo

Fine-tuna un ``VisionTeacher`` (ResNet-50 ImageNet-pretrained) sui frame video
centrali del subset VGGSound, con sola cross-entropy loss. Il checkpoint
risultante (``best.pth``) sarà usato come teacher frozen nella distillation
(Fase 3) e come termine di confronto nella valutazione comparativa (Fase 4).
...
Uso (cluster, via SLURM):
    sbatch experiments/scripts/submit_job.sh train_teacher \\
        --config experiments/configs/teacher_vision.yaml

Resume da checkpoint:
    python -m src.training.train_teacher \\
        --config experiments/configs/teacher_vision.yaml \\
        --resume experiments/checkpoints/teacher_vision/best.pth

Argomenti CLI:
    --config     Path al file YAML di configurazione (default: experiments/configs/teacher_vision.yaml).
...
def train_one_epoch(
    ...
    for batch_idx, (frame, labels) in enumerate(loader):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train ResNet-50 Vision Teacher — Track 24")
    parser.add_argument(
        "--config",
        default="experiments/configs/teacher_vision.yaml",
...
def main() -> None:
    ...
    output_dir = Path(args.output_dir) if args.output_dir else (
        _PROJECT_ROOT / "experiments" / "checkpoints" / "teacher_vision"
    )
    log_dir = _PROJECT_ROOT / "experiments" / "logs" / "teacher_vision"
    ...
    # Modalità "video": il dataset restituisce (frame, label) e applica già la
    # data augmentation (RandomResizedCrop/Flip/ColorJitter) in train e la
    # pipeline deterministica (Resize/CenterCrop) in val, con normalizzazione
    # ImageNet richiesta dal backbone pretrained.
    ...
    # -- Modello --------------------------------------------------------
    model = build_vision_teacher(cfg).to(device)
    logger.info("%s", model)
    ...
    # SGD con momentum solo sui parametri trainabili (il backbone congelato resta fuori).
    ...
    if scheduler_name == "step":
        scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer,
            step_size=int(train_cfg.get("step_size", 7)),
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        # Step dello scheduler (StepLR avanza una volta per epoca).
