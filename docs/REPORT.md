...
A telling detail is the disagreement between the validation and test rankings. On validation, EXP-004 ($\alpha=0.5$, 88.91%) is the strongest distilled model, but on the blind test set EXP-005 ($\alpha=0.7$) takes the lead at 66.99%. Selecting on validation alone would therefore have chosen a suboptimal hyperparameter, which is precisely why we reserved a strictly held-out test split for the final decision.
